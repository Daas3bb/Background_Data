...
path = Path(r"d:\项目\CEO\数据后台\index.html")
text = path.read_text(encoding="utf-8")
start = text.index("      function renderGameActivityDauTrend() {")
end = text.index("      function renderGameParticipationDauBetChart() {")

new = r'''      function renderGameActivityBetPlayerTrend() {
        const dayCount = 30;
        ...
        const totalDau = newPlayers.map((v, i) => v + oldDau[i]);
        return renderMultiLineTrendChart({
          labels,
          series: [
            { name: "投注玩家DAU", data: totalDau, color: "#165DFF" },
            { name: "新增投注玩家", data: newPlayers, color: "#14C9C9" },
      ...
      function renderGameActivityCharts() {
        ...
        <div class="game-metric-charts" id="gameActivityCharts">
          ${renderGameMetricChartCard(
            "bet-trend",
            "投注玩家趋势",
            "投注玩家DAU，新增投注玩家，老投注玩家DAU",
            "近30日投注玩家DAU / 新增投注玩家 / 老投注玩家DAU 变化",
